...
def get_data(filters):
	...
	data = frappe.db.sql(""" SELECT ss.employee, ss.employee_name,  ss.designation, ss.branch, sd.amount
				FROM `tabSalary Slip` AS ss , `tabSalary Detail` AS sd, `tabEmployee` AS e
				WHERE ss.name= sd.parent AND ss.employee = e.employee
				AND sd.salary_component ='Mess Deductions' {0} and ss.docstatus =1""".format(conditions))
