	...
	def update_deduction_balance(self):
		for ssl in self.deductions:
			if (ssl.ref_docname and ssl.amount and ssl.total_deductible_amount):
				sst = frappe.get_doc("Salary Detail", {"name":ssl.ref_docname})
				if sst:
						...
						sst.save()
			if (ssl.salary_component == 'EMI Sales' and ssl.amount and ssl.reference_number):
				ref_doc = frappe.get_doc("EMI Sales",ssl.reference_number)
				if self.docstatus == 1 and ref_doc.status != 'Paid' and ref_doc.docstatus == 1 :
					# bal = ssl.total_outstanding_amount
					# if flt(ref_doc.outstanding_amount) == ssl.total_outstanding_amount:
					# 	bal = flt(ssl.total_outstanding_amount)
					# else:
					bal = flt(ref_doc.outstanding_amount)-flt(ssl.amount)
					if (ssl.ref_docname and ssl.amount and ssl.total_deductible_amount):
						frappe.db.sql("""
							update `tabSalary Detail` set total_outstanding_amount = {},
							total_deducted_amount = {}
							where name = '{}'
						""".format(flt(bal), flt(ref_doc.total_receivable_amount)-flt(bal), ssl.ref_docname))
					...
					if (ssl.ref_docname and ssl.amount and ssl.total_deductible_amount):
						frappe.db.sql("""
							...
							where name = '{}'
						""".format(flt(bal), flt(ref_doc.total_receivable_amount)-flt(bal), ssl.ref_docname))
				# frappe.db.set_value("EMI Sales", ssl.reference_number, 'outstanding_amount', bal)
