...
class SalarySlip(TransactionBase):
	...
	def pull_sal_struct(self, ss_doc, calc_days=None):
		...
		basic_pay_arrears = flt(
			getattr(self, "basic_pay_arrears", 0)
		)

		# Get Employer PF % from Employee Group
		employee_group = frappe.db.get_value(
			"Employee",
			self.employee,
			"employee_group"
		)
		zero_employer_pf_groups = {
			"Officer (RBA)",
			"Troops (RBA)",
			"Contract (RBA)",
			"RBA-Deceased (2003)",
		...
		if employee_group in zero_employer_pf_groups:
			self.employer_pf = 0
		else:
			employer_percent = flt(
				frappe.db.get_value(
					"Employee Group",
					employee_group,
					"employer_pf"
			...
			self.employer_pf = round(
				(basic_amt + basic_pay_arrears)
				* employer_percent
				* 0.01
			)
		...
		if self.salary_slip_based_on_timesheet:
			...
			self.total_working_hours = sum(
				[d.working_hours or 0.0 for d in self.timesheets]
			) or 0.0
			self.add_earning_for_hourly_wages(ss_doc.salary_component)
