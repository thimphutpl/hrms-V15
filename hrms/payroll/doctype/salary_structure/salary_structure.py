...
class SalaryStructure(Document):
		...
		for parentfield in ['earnings', 'deductions']:
			...
			for i in self.get(parentfield):
				...
				if i.salary_component is not None:
					...
					if i.salary_component in ('Basic Salary') and i.salary_component in list(dup.keys()):
	...
	# 	frappe.throw(frappe.as_json(emp))
	...
	@frappe.whitelist()
	def get_employee_details(self):
		"""Fetch and set employee-related details when an employee is selected."""
		if not self.employee:
			frappe.throw(_("Please select an Employee."))

		emp = frappe.get_doc("Employee", self.employee)

		# Optional: show full employee JSON for debugging
		# frappe.msgprint(frappe.as_json(emp, indent=4))

		# Assign values from Employee doctype
		self.employee_name = emp.employee_name
		self.branch = emp.branch
		self.designation = emp.designation
		self.employment_type = emp.employment_type
		...
		self.department = emp.department
		self.business_activity = emp.business_activity
		self.cost_center = emp.cost_center if hasattr(emp, "cost_center") else None
		self.backup_employee = self.employee
	...
	def validate_amount(self):
		...
		if  self.eligible_for_fixed_allowance:
			self.eligible_for_fixed_allowance = frappe.db.get_value("Employee Grade",self.employee_grade, "fixed_allowance")
...
def make_salary_slip(source_name, target_doc=None, calc_days={}):
	def postprocess(source, target):
		...
		for e in calc_map['earnings']:
			if e['salary_component'] == 'Basic Salary':
...
def get_basic_and_gross_pay(employee, effective_date=today()):
	struc = frappe.db.sql(""" select sst.name,
			sum(case when sd.salary_component = "Basic Salary" then coalesce(sd.amount,0) else 0 end) basic_pay,
			sum(case when (sc.type = 'Earning' and (sd.salary_component = "Basic Salary" or coalesce(sc.field_name,'') != '')) then ifnull(sd.amount,0) else 0 end) gross_pay
