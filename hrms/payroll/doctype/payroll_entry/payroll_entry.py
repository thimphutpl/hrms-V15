...
class PayrollEntry(Document):
	...
	def make_bank_entry(self):
		...
		for salary_detail in salary_details:
			...
			if salary_detail.salary_component != "EMI Sales":
				...
				reference_doc = frappe.get_doc(salary_detail.reference_type, salary_detail.reference_number)
				interest_amount = frappe.db.sql("select interest from `tabEMI Payment Schedule` where parent = '{}' and month(due_date) = {} and year(due_date) = {}".format(salary_detail.salary_structure, salary_detail.month, salary_detail.fiscal_year),as_dict=1)
				income_interest_account = frappe.db.get_value("Company", self.company, "default_income_interest_account")
				...
				if not income_interest_account:
					frappe.throw("Please set Default Income from Interest Account in Company Accounts Settings.")
				if not interest_interest_account:
					frappe.throw("Please set Default Interest Income Account (Accrued Account) in Company Accountsn Settings.")

				if interest_amount:
					interest_amount = interest_amount[0].interest
				else:
					interest_amount = 0
				posting.setdefault("to_payables", []).append({
					"account"        : salary_detail.gl_head,
					"debit_in_account_currency": flt(flt(salary_detail.amount)-flt(interest_amount),2),
					"debit": flt(flt(salary_detail.amount)-flt(interest_amount),2),
					"against_account": default_payable_account,
					"cost_center"    : salary_detail.cost_center,
					"party_check"    : 0,
					"account_type"   : salary_detail.account_type if salary_detail.party_type == "Employee" else "",
				...
				posting.setdefault("to_payables", []).append({
					...
					"debit_in_account_currency":flt(interest_amount,2),
					"debit":flt(interest_amount,2),
					"against_account": default_payable_account,
					"cost_center"    : salary_detail.cost_center,
					"party_check"    : 0,
					"account_type"   : salary_detail.account_type if salary_detail.party_type == "Employee" else "",
				...
				posting.setdefault("to_payables", []).append({
					...
					"credit_in_account_currency": flt(flt(salary_detail.amount)-flt(interest_amount),2),
					"debit": flt(flt(salary_detail.amount)-flt(interest_amount),2),
					"against_account": default_payable_account,
					"cost_center"    : salary_detail.cost_center,
					"party_check"    : 0,
				...
				posting.setdefault("to_payables", []).append({
					"account"        : income_interest_account,
					"credit_in_account_currency":flt(interest_amount,2),
					"credit":flt(interest_amount,2),
					"against_account": default_payable_account,
					"cost_center"    : salary_detail.cost_center,
					"party_check"    : 0,
					"account_type"   : salary_detail.account_type if salary_detail.party_type == "Employee" else "",
