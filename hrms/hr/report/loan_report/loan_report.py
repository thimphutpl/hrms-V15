...
def get_data(filters):
	...
	# 		t2.reference_type, t2.bank_name, t2.reference_number, t2.amount, t2.total_deductible_amount, t2.total_outstanding_amount,
	...
	# 	and case when t2.bank_name = 'RICBL' then t2.reference_type like '%loan%' else 1 = 1 end
	# 	and exists
	# 		(select 1
	...
	data = frappe.db.sql("""
		select t1.employee, t3.employee_name, t3.passport_number, t1.designation,
			t2.reference_type, t2.bank_name, t2.reference_number, t2.amount, t2.total_deductible_amount, t2.total_outstanding_amount,
			t1.company, t1.cost_center, t1.branch, t1.department,  t1.section,t1.section,
			t1.fiscal_year, t1.month
		from `tabSalary Slip` t1, `tabSalary Detail` t2, `tabEmployee` t3
		where t1.docstatus = 1 {}
		and t3.employee = t1.employee
...
def get_conditions(filters):
	...
	if filters.get("bank"): conditions += "and t2.bank_name = '{0}'".format(filters.bank)
