...
class InterestFree(Document):
	...
	def validate(self):
		...
		self.validate_employment_duration()
	def validate_employment_duration(self):
		...
		today = getdate()
		years_of_service = (today - joining_date).days / 365
	
		
		if years_of_service < 3:
			frappe.throw(
				_("The employee must have completed at least 3 years of service. "
				" Current service duration: {0:.2f} years").format(years_of_service),
				title=_("Ineligible Employee")
			)
	...
	def set_pay_details(self):
		...
		self.gross_pay = flt(earnings.get("total_earning", 0))*12
		self.basic_pay = flt(earnings.get("basic_pay", 0))
		self.net_pay = flt(earnings.get("net_pay", 0))
		self.advance_amount = self.gross_pay
	...
	def calculate_amount(self):
		try:
			...
			max_amount = flt(self.gross_pay) 
			
			if flt(self.advance_amount) > max_amount:
				frappe.throw(
					_("The loan amount cannot exceed your one-year gross pay."
