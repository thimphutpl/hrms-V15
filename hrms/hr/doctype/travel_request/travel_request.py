...
class TravelRequest(AccountsController):
	...
	def make_advance_payment(self):
		default_employee_advance_account = frappe.get_value("Company", self.company, "default_employee_advance_account")
		if not default_employee_advance_account:
			frappe.throw("Please set Default Employee Advance Accoutn in {}".format(
	...
	def get_advances(self):
		...
		query = frappe.qb.from_(advance).select(
			...
			& (advance.reference_name == self.name)
		)
