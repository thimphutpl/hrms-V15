...
class ForeignLabourer(Document):
	...
	#added by cety
@frappe.whitelist()
def rejoin_foreign_labourer(docname):
	doc = frappe.get_doc("Foreign Labourer", docname)
	if doc.docstatus != 1 or doc.status != "Left":
		frappe.throw("Only submitted and 'Left' employees can be rejoined.")
	frappe.db.sql("""
		update `tabForeign Labourer` set docstatus=0, status='At-Site', date_of_separation=NULL where name=%s
	""", (docname,))
