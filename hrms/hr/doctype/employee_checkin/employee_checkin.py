...
# 				out_time = out_log.time
# 				total_hours += time_diff_in_hours(in_log.time, out_log.time)
# 	if shift_end and out_time and out_time > shift_end:
# 		overtime_hours = time_diff_in_hours(shift_end, out_time)					
# 	return total_hours,overtime_hours, in_time, out_time
...
def mark_attendance_and_link_log(
	...
	if not first_in_log or not last_out_log:
		return None
...
# def calculate_working_hours(logs, check_in_out_type, working_hours_calc_type, shift_end=None):
...
# 		reference_date = getdate(logs[0].time) if logs else getdate()
# 		shift_end = get_datetime(datetime.combine(reference_date, shift_end_time))
# 	if check_in_out_type == "Alternating entries as IN and OUT during the same shift":
# 		in_time = logs[0].time
# 		if len(logs) >= 2:
# 			out_time = logs[-1].time
...
# 					total_hours += time_diff_in_hours(start, shift_end)
# 					overtime_hours += time_diff_in_hours(shift_end, end)
# 				else:
# 					total_hours += time_diff_in_hours(start, end)
# 				if not in_time:
# 					in_time = start
# 				out_time = end
...
# 	# 				overtime_hours = time_diff_in_hours(shift_end, out_time)
# 	# 				total_hours = time_diff_in_hours(in_time, shift_end)
# 	return total_hours,overtime_hours, in_time, out_time
def calculate_working_hours(logs, check_in_out_type, working_hours_calc_type, shift_start=None, shift_end=None):
	total_hours = 0
	overtime_hours = 0
	...
	# Reference date
	reference_date = logs[0].time.date() if logs else date.today()

	# Convert shift_start string to datetime
	if shift_start:
		if isinstance(shift_start, str):
			shift_start = datetime.combine(reference_date, datetime.strptime(shift_start, "%H:%M:%S").time())
		elif isinstance(shift_start, timedelta):
			shift_start = datetime.combine(reference_date, datetime.min.time()) + shift_start
		elif isinstance(shift_start, datetime):
			pass
		else:
			raise ValueError(f"Unsupported shift_start type: {type(shift_start)}")

	# Normalize shift_end to datetime
	if shift_end:
		if isinstance(shift_end, str):
			shift_end = datetime.combine(reference_date, datetime.strptime(shift_end, "%H:%M:%S").time())
		elif isinstance(shift_end, timedelta):
			shift_end = datetime.combine(reference_date, datetime.min.time()) + shift_end
		elif isinstance(shift_end, datetime):
			pass
		else:
			raise ValueError(f"Unsupported shift_end type: {type(shift_end)}")
	# Alternating IN/OUT entries
	# -------------------------
	if check_in_out_type == "Alternating entries as IN and OUT during the same shift":
		in_time = logs[0].time
		out_time = logs[-1].time if len(logs) >= 2 else in_time

		if working_hours_calc_type == "First Check-in and Last Check-out":
			# Morning extra
			if shift_start and in_time < shift_start:
				morning_extra_hours = time_diff_in_hours(in_time, shift_start)
			# Overtime
			if shift_end and out_time > shift_end:
				overtime_hours = time_diff_in_hours(shift_end, out_time)
			# Total hours = only within shift
			total_hours = time_diff_in_hours(max(in_time, shift_start), min(out_time, shift_end))

		elif working_hours_calc_type == "Every Valid Check-in and Check-out":
			logs_copy = logs[:]
			while len(logs_copy) >= 2:
				start = logs_copy[0].time
				end = logs_copy[1].time
				# Morning extra
				if shift_start and start < shift_start:
					morning_extra_hours += time_diff_in_hours(start, shift_start)
				# Overtime
				if shift_end and end > shift_end:
					overtime_hours += time_diff_in_hours(shift_end, end)
				# Total hours within shift
				total_hours += time_diff_in_hours(max(start, shift_start), min(end, shift_end))
				del logs_copy[:2]

	# -------------------------
	# Strictly based on Log Type
	# -------------------------
	elif check_in_out_type == "Strictly based on Log Type in Employee Checkin":
		if working_hours_calc_type == "First Check-in and Last Check-out":
			first_in_index = find_index_in_dict(logs, "log_type", "IN")
			last_out_index = find_index_in_dict(list(reversed(logs)), "log_type", "OUT")

			first_in_log = logs[first_in_index] if first_in_index is not None else None
			last_out_log = logs[len(logs) - 1 - last_out_index] if last_out_index is not None else None
	...
	return total_hours, overtime_hours, morning_extra_hours, in_time, out_time
