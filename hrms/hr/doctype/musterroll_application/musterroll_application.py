	...
	def create_mr(self):
		for a in self.items:
			...
			if a.approver_status == 'Approved':
				...
				doc.rate_per_hour_normal = a.rate_per_hour_normal
				doc.company       = "Construction Development Corporation Limited"
				doc.id_card       = cid
				#frappe.throw(doc.temp_doctype)
				doc.designation = a.designation
				doc.bank_name = a.bank
				#doc1=frappe.get_doc("Muster Roll Employee",cid)
				#frappe.throw(doc1.person_name)
				...
				child_entry_exists = False
				for child in doc.get('musterroll'):  # 'musterroll' is the child table
					if child.parent == a.citizenship_id:  # Match the child based on citizenship_id
						child.rate_per_day = a.rate_per_day  # Update per_rate field in the child table
						child_entry_exists = True  # Mark that an entry has been updated

				if not child_entry_exists:
               
					child = doc.append('musterroll', {})
					child.rate_per_day = a.rate_per_day  # Set per_rate for the new row
					child.parent = a.citizenship_id  # Set unique identifier
					#child.some_other_field = a.some_other_value
