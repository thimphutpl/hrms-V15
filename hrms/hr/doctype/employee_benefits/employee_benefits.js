...
frappe.ui.form.on('Employee Benefits', {
	...
	//added by Kinzang.N on 27/10/2025
	// validate: function (frm) {
	// 	if (flt(frm.doc.total_deducted_amount) >= flt(frm.doc.total_amount)) {
	// 		frappe.throw(__("Total Deducted Amount must be less than Total Amount."));
	// 	}
	// }
...
frappe.ui.form.on("Separation Item", {
	...
	"benefit_type": function (frm, cdt, cdn) {
		...
		var item = locals[cdt][cdn]
		if (item.benefit_type == "Transfer Grant(Normal)" || item.benefit_type == "Provision for Travel Allowance(Repatriation)" || item.benefit_type == "Separation for transfer Grant") {
			return frappe.call({
		...
		} else if (item.benefit_type == "LTC") {
			return frappe.call({
				method: "hrms.hr.doctype.employee_benefits.employee_benefits.get_ltc_amount",
				args: { "employee": frm.doc.employee, "date": frm.doc.posting_date },
				callback: function (r) {
					console.log(r.message);
					if (r.message) {
		...
		} else if (item.benefit_type == "PF-Employee") {
			return frappe.call({
				method: "hrms.hr.doctype.employee_benefits.employee_benefits.get_pf_employee",
				args: { "employee": frm.doc.employee },
				callback: function (r) {
					console.log(r.message);
					if (r.message) {
		...
		} else if (item.benefit_type == "PF-Employer") {
			return frappe.call({
				method: "hrms.hr.doctype.employee_benefits.employee_benefits.get_pf_employer",
				args: { "employee": frm.doc.employee },
				callback: function (r) {
					console.log(r.message);
					if (r.message) {
...
var set_total = function (frm) {
	...
	// Validation: total deduction must be less than total earning
	// if (total_deduction >= total_earning) {
	// 	frappe.msgprint({
	// 		title: __("Invalid Amount"),
	// 		message: __("Total Deducted Amount must be less than Total Amount."),
	// 		indicator: "red"
	// 	});
	// }
