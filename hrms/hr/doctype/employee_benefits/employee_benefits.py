...
class EmployeeBenefits(Document):
    ...
    def on_submit(self):
        ...
        frappe.enqueue(
            'hrms.hr.doctype.employee_benefits.employee_benefits.post_journal',
            docname=self.name,
            queue='long'
        )
    ...
    #     je.cost_center = emp.cost_center
    ...
    #         frappe.throw(_("Please set Default Bank Account in Company: {0}").format(self.company))

    #     tax_account = frappe.db.get_value("Company", self.company, "default_salary_payable_account")
    #     if not tax_account:
    #         frappe.throw(_("Please set Default Salary Payable Account in Company: {0}").format(self.company))
            
    #     total_amount = tax_amount = 0
    #     # Benefits
    #     for a in self.items:
    #         #frappe.msgprint(str(a.gl_account))
    #         if not flt(a.amount):
    #             continue
    ...
    #     # Credit Account
    #     payable_account = frappe.db.get_value("Company", self.company, "default_salary_payable_account")
    ...
    #     je_references = str(je.name)
        ...
        if flt(total_amount):
            ...
            jeb.cost_center = emp.cost_center
            jeb.company = emp.company
            jeb.append("accounts", {
                "account": payable_account,
...
def post_journal(docname):
    doc=frappe.get_doc("Employee Benefits",docname)
    emp = frappe.get_doc("Employee", doc.employee)
    #frappe.throw(str(doc.employee))
    ...
    je.title = str(doc.purpose) + " Employee Benefits Payable (" + str(doc.employee_name) + ")"
    je.voucher_type = "Journal Entry"
    je.naming_series = "Journal Voucher"
    je.remark = str(doc.purpose) + 'Benefit payments for ' + str(doc.employee_name) + "("+str(doc.employee)+")"
    je.company = emp.company
    #expense_bank_account = get_bank_account(self.branch)
    expense_bank_account=frappe.db.get_value("Company", doc.company, "default_bank_account")
    #frappe.throw(str(self.company))
    if not expense_bank_account:
        frappe.throw(_("Please set Default Bank Account in Company: {0}").format(doc.company))

    tax_account = frappe.db.get_value("Company", doc.company, "default_salary_payable_account")
    if not tax_account:
        frappe.throw(_("Please set Default Salary Payable Account in Company: {0}").format(doc.company))
    ...
    # Deductions
    for b in doc.deduction_details:
        if not flt(b.amount):
            continue
    ...
    # Credit Account
    payable_account = frappe.db.get_value("Company", doc.company, "default_salary_payable_account")
    if not payable_account:
        frappe.throw(_("Please set Default Salary Payable Account in Company: {0}").format(doc.company))
    ...
    if flt(total_amount):
        jeb = frappe.new_doc("Journal Entry")
        jeb.flags.ignore_permissions = 1
        jeb.title = doc.purpose+" Employee Benefits Payment(" + doc.employee_name + "  " + doc.name + ")"
        jeb.voucher_type = "Bank Entry"
        jeb.naming_series = "Bank Payment Voucher"
        jeb.remark = 'Benefit payment against : ' + doc.name
        jeb.posting_date = doc.posting_date
        jeb.branch = emp.branch
        jeb.cost_center = emp.cost_center
        jeb.company = emp.company
        jeb.append("accounts", {
    ...
    doc.db_set("journal", je_references)

@frappe.whitelist()
def get_gl_head(type,company,is_deduction):
    #frappe.throw(str(is_deduction))
    doc = 'Benefit Type Account' if is_deduction == "0" else 'Deduction Type Account'
    #if is_deduction=="0":
    account=frappe.db.sql(f""" select account from `tab{doc}` where parent=%s and company=%s limit 1 """,(type,company),as_dict=1)
    if not account:
        frappe.throw("set Account")
    # return account
    return {
    "account": account[0].get("account")
    }
