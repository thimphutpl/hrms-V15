...
erpnext.attendance_tool_others = {
	load_employees: function(frm) {
		if(frm.doc.date && frm.doc.cost_center && frm.doc.branch && frm.doc.employee_type) {
			frappe.call({
				method: "hrms.hr.doctype.attendance_tool_others.attendance_tool_others.get_employees",
				args: {
					date: frm.doc.date,
	...
	make: function(frm, employee) {
		...
		mark_employee_toolbar.find(".btn-mark-present")
			...
			.on("click", function() {
				...
				frappe.call({
					method: "hrms.hr.doctype.attendance_tool_others.attendance_tool_others.mark_employee_attendance",
					args:{
