...
class BulkLeaveEncashment(Document):
	...
	def calculate_amount(self):
		...
		for d in self.items:
			total_encashment_amount += flt(d.encashment_amount)
	...
	def create_leave_ledger_entry(self, submit=True):
		for d in self.items:
			if d.leave_balance <= 30 and d.leave_balance > 0 and d.casual_leave_balance == 0:
				args = frappe._dict(
					employee=d.employee,
					employee_name=d.employee_name,
			...
			elif d.leave_balance < 30 and d.casual_leave_balance > 0:
				args = frappe._dict(
					employee=d.employee,
					employee_name=d.employee_name,
					leaves= d.leave_balance * -1,
					from_date=self.encashment_date,
					to_date=self.encashment_date,
					is_carry_forward=0
				)
				args2 = frappe._dict(
					employee=d.employee,
					employee_name=d.employee_name,
					leaves= (d.encashable_days-d.leave_balance) * -1,
					from_date=self.encashment_date,
					to_date=self.encashment_date,
					is_carry_forward=0
				)
				create_leave_ledger_entry(self, args, submit)
				create_leave_ledger_entry(self, args2, submit, leave_type = "Casual Leave")
			elif d.leave_balance == 0 and d.casual_leave_balance > 0:
				args = frappe._dict(
					employee=d.employee,
					employee_name=d.employee_name,
					leaves= d.encashable_days * -1,
				...
				create_leave_ledger_entry(self, args, submit)
	def get_leave_detail_from_ler(self):
		for emp in self.items:
			pay = get_basic_and_gross_pay(employee=emp.employee, effective_date=today())
			emp.encashable_days = 0
			days = frappe.db.sql('''
                            select encashment_days, earned_leave_balance,casual_leave_balance from `tabLeave Encashment Request` 
                            where ref_id='{0}' and employee='{1}' and leave_period='{2}';
                                       '''.format(self.leave_encashment_request,emp.employee,self.leave_period)
			)
   
			# frappe.throw(frappe.as_json(days[0][0]))
			emp.encashable_days = days[0][0]
			
			basic_pay = flt(pay.get("basic_pay") or 0)
			basic_pay_month = basic_pay / 30
			emp.encashment_amount = flt((basic_pay_month * flt(emp.encashable_days)), 2)
			
			emp.encashment_tax = flt(get_salary_tax(emp.encashment_amount), 2)
			emp.payable_amount = flt((emp.encashment_amount - emp.encashment_tax), 2)
