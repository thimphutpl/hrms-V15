...
class EmployeeAdvance(Document):
	...
	def select_advance_account(self):
		if self.advance_type == "Salary Advance":
			...
			self.advance_account = frappe.db.get_value("Company", self.company, "default_travel_advance_account")
	...
	def update_reference(self, cancel = 0):
		if self.advance_type == "Travel Advance" and cancel == 0:
			if not frappe.db.get_value("Travel Request",self.reference_name,"employee_advance_reference") and self.status == "Paid":
				frappe.db.sql(""" 
					update `tabTravel Request`
					set employee_advance_reference = '{0}'
					where name = '{1}'
				""".format(self.name,self.reference_name))
	...
	def check_duplicate_advance(self):
		if frappe.db.sql("""
				select count(reference_name) 
				from `tabEmployee Advance` 
				where reference_name = '{}'
				and name != '{}'
				and docstatus != 2
			""".format(self.reference_name, self.name))[0][0] >= 1 :
		...
		if self.reference_type == "Travel Request":
			...
			if self.docstatus == 2:
				...
				frappe.db.sql("""
					...
					advance_amount = {}
					where name = '{}'
				""".format(advance_amount, self.reference_name))
				if advance_amount != 0:
