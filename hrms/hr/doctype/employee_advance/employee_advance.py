...
class EmployeeAdvance(Document):
	...
	@frappe.whitelist()
	def calculate_amount(self, validate=0):
		advance_account = frappe.db.get_value("Company", self.company, "default_employee_advance_account")
		deduction = 0.0
		...
		if flt(self.advance_amount) > flt(max_amount) and validate == 1:
			frappe.throw(
				_(
					"The advance amount cannot exceed {} times your net pay. "
					"You are attempting to set an advance of {} while the maximum allowable amount is {}. "
					"Please adjust the advance amount accordingly.".format(max_month,
