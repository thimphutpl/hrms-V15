...
class LeaveControlPanel(Document):
	...
	# ---------------------------------------------------------------------
	# LEAVE TYPE BASED (uses Leave Type → Applicable Gender)
	# ---------------------------------------------------------------------
	def create_leave_allocations(self, employees: list) -> dict:
		from_date, to_date = self.get_from_to_date()
		success, failure = [], []
		...
		for employee in employees:
			try:
				...
				frappe.db.rollback(save_point=savepoint)
				frappe.log_error(frappe.get_traceback(), f"Leave Allocation failed for {employee}")
				failure.append(employee)
	...
	# ---------------------------------------------------------------------
	# 🔥 LEAVE POLICY BASED (uses Policy Detail → Applicable Gender)
	# ---------------------------------------------------------------------
	def create_leave_policy_assignments(self, employees: list) -> dict:
		from_date, to_date = self.get_from_to_date()
		assignment_based_on = None if self.dates_based_on == "Custom Range" else self.dates_based_on
		...
		for employee in employees:
			try:
				...
				assignment.effective_to = to_date
				assignment.leave_period = self.leave_period
				assignment.carry_forward = self.carry_forward
				...
				if invalid_leave_types:
					frappe.db.delete(
						...
						{
							"employee": employee,
							"leave_type": ["in", invalid_leave_types],
							"docstatus": 1,
						},
					)
				...
				frappe.log_error(
					...
					f"Leave Policy Assignment failed for {employee}"
				)
				failure.append(employee)
	...
	def get_latest_leave_period(self):
		return frappe.db.get_value(
			"Leave Period",
			{"is_active": 1, "company": self.company or get_default_company()},
			"name",
			order_by="from_date desc",
		)
	@frappe.whitelist()
	...
	def get_filters(self):
		filter_fields = [
			"company", "employment_type", "branch",
			"department", "designation", "employee_grade"
		]
		filters = [["status", "=", "Active"]]
